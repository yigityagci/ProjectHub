...
interface RegistrationTokenSummary {
  ...
  createdByIsDeleted: boolean;
  expiresAt: string;
  usedAt: string | null;
  usedByEmail: string | null;
  usedByDisplayName: string | null;
  usedByIsDeleted: boolean;
...
function tokenStatus(token: RegistrationTokenSummary): string {
  ...
  if (token.usedAt) {
    const who = token.usedByDisplayName
      ? formatUserName(token.usedByDisplayName, token.usedByIsDeleted)
      : token.usedByEmail ?? "someone";
            ...
            <li key={token.id}>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.1rem", minWidth: 0 }}>
                ...
                <span style={{ fontSize: "0.78rem", color: "var(--ph-muted)" }}>
                  Generated by {formatUserName(token.createdByDisplayName, token.createdByIsDeleted)} ·{" "}
                  {formatDate(token.createdAt)} · Expires{" "}
